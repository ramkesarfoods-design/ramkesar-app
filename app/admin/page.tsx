...
import { 
  ...
  Volume2, VolumeX, ChevronRight, Activity, ShieldCheck, Moon, Sun, AlertTriangle, Printer, FileText
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    return () => { unsub(); unsubMenu(); unsubRiders(); unsubAds(); clearInterval(timer); };
  }, [soundEnabled, soundType, orders.length]);

  // Auto Schedule Logic Loop
  useEffect(() => {
    ...
    const interval = setInterval(() => {
        ...
        if (storeStatus !== shouldBeOpen) {
            setStoreStatus(shouldBeOpen);
        }
    }, 60000);
    return () => clearInterval(interval);
  }, [autoSchedule, openTime, closeTime, storeStatus]);
  ...
  const stats = useMemo(() => {
    ...
    const calculateGrandTotal = (order: any) => {
        ...
        const itemsTotal = order.items?.reduce((acc:any, i:any) => acc + (i.price * i.qty), 0) || 0;
    ...
    return {
      ...
      active: orders.filter(o => ["Pending", "Confirmed", "Preparing", "Out for Delivery"].includes(o.status)).length, // ADDED "Confirmed"
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(o => {
      const isLive = ["Pending", "Confirmed", "Preparing", "Out for Delivery"].includes(o.status); // ADDED "Confirmed"
      const matchesFilter = 
  ...
  const updateStatus = async (orderId: string, newStatus: string, riderId?: string) => {
    ...
    try {
      ...
      // --- NEW: DISPATCH COLLECTION LOGIC FOR RIDER PANEL ---
    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${isDarkMode ? 'dark' : ''}`}>
      ...
      <aside className="w-72 bg-slate-100 border-r border-slate-200 flex flex-col shrink-0 z-50 inner-content">
        ...
        <div className="p-6">
            <div className="bg-white rounded-3xl p-4 border border-slate-200 shadow-sm">
                <button onClick={() => setStoreStatus(!storeStatus)} className={`w-full flex items-center justify-center gap-2 py-3 rounded-2xl font-black text-[10px] uppercase italic transition-all ${storeStatus ? 'bg-green-500 text-white' : 'bg-red-600 text-white'}`}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative inner-content">
        ...
        <header className="h-24 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-10 shrink-0 sticky top-0 z-40">
          <div>
            <h2 className="text-xl font-black italic uppercase tracking-tighter text-slate-900">{activeTab}</h2>
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search Orders..." 
                className="w-80 pl-12 pr-6 py-3 bg-slate-50 rounded-2xl outline-none text-[11px] font-bold border focus:border-red-100 transition-all" />
            </div>
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3 bg-slate-900 text-white p-1.5 pr-5 rounded-full">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key="orders" className="space-y-8">
                ...
                <div className="grid grid-cols-3 gap-8">
                  {filteredOrders.map(order => {
                    ...
                    const isDelayed = ["Pending", "Confirmed"].includes(order.status) && (currentTime - orderTimestampMs > 600000); // ADDED "Confirmed"
                    ...
                    <div key={order.id} className={`bg-white rounded-[2.5rem] border ${isDelayed ? 'border-red-400 shadow-red-100' : 'border-slate-100 shadow-sm'} p-7 flex flex-col group relative overflow-hidden transition-all`}>
                      ...
                      <div className="mt-auto space-y-2">
                        {/* ADDED "Confirmed" HERE SO BUTTONS SHOW UP */}
                        ...
                        {order.status === "Preparing" && (
                            <div className="space-y-2 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                                <select onChange={(e) => handleRiderSelect(order.id, e.target.value)} className="w-full bg-white p-3 rounded-xl text-[10px] font-black uppercase italic outline-none border focus:border-red-100 text-slate-600 cursor-pointer">
                                   ...
                                   {riders.map(r => <option key={r.id} value={r.id}>{r.name} ({r.vehicleNumber || 'No Vehicle'})</option>)}
                                </select>
                                <button 
                     ...
                     <div key={item.id} className="bg-white rounded-[2.5rem] border border-slate-100 p-5 shadow-sm group">
                        ...
                        <div className="flex justify-between items-end mt-4 pt-4 border-t border-slate-50">
                           ...
                           <div className="flex gap-2">
                             <button onClick={() => {setEditingItem(item); setShowItemModal(true);}} className="p-2.5 bg-slate-50 rounded-xl text-slate-400 hover:text-blue-500"><Edit3 size={16}/></button>
                             <button onClick={() => updateDoc(doc(db, "menus", item.id), { inStock: !item.inStock })} className={`p-2.5 rounded-xl ${item.inStock ? 'bg-green-50 text-green-500' : 'bg-red-50 text-red-500'}`}><Power size={16}/></button>
                           </div>
                        </div>
                     </div>
                   )})}
            ...
            {activeTab === "riders" && (
                <div className="grid grid-cols-3 gap-10">
                    <div className="col-span-2 space-y-6">
                        <h2 className="text-2xl font-black italic text-slate-900 uppercase tracking-tighter">Delivery Fleet</h2>
                        <div className="grid grid-cols-2 gap-6">
                            {riders.map(r => (
                                <div key={r.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5">
                                    <div className="w-14 h-14 bg-slate-900 text-white rounded-3xl flex items-center justify-center shadow-lg shadow-slate-200"><Bike size={24}/></div>
                                    <div className="flex-1 overflow-hidden">
                                        <p className="font-black italic uppercase text-sm text-slate-900 truncate">{r.name}</p>
                                        <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">ID: {r.id}</p>
                                        <p className="text-[9px] font-black text-slate-500 uppercase mt-0.5 bg-slate-50 inline-block px-2 py-0.5 rounded-md border">{r.vehicleNumber}</p>
                                    </div>
                                    <button onClick={() => deleteDoc(doc(db, "riders", r.id))} className="p-3 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={16}/></button>
                                </div>
                            ))}
                        </div>
                    </div>
                    <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl h-fit">
                        <h3 className="font-black italic uppercase text-xs text-slate-400 mb-8 text-center tracking-widest underline underline-offset-8">Rider Onboarding</h3>
                        <form onSubmit={async (e:any) => {
                            ...
                            <input name="vehicleNumber" placeholder="Vehicle Reg No. (UP16-XX-XXXX)" className="w-full bg-slate-50 p-4 rounded-2xl text-[11px] font-bold outline-none border focus:border-red-100 text-slate-900" required />
                            <button className="w-full bg-red-600 text-white py-5 rounded-2xl font-black text-[11px] uppercase italic shadow-2xl active:scale-95 transition-all">Verify & Approve</button>
                        </form>
                    </div>
                </div>
            )}
                ...
                <div className="max-w-2xl bg-white p-12 rounded-[4rem] border border-slate-100 shadow-sm mx-auto">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between p-7 bg-slate-50 rounded-[2.5rem]">
                            ...
                            <button onClick={() => setStoreStatus(!storeStatus)} className={`w-14 h-7 rounded-full relative transition-all ${storeStatus ? 'bg-green-500' : 'bg-red-500'}`}>
